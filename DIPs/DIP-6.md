...
This solution can also convert non-crypto users into Ethereum users, as attendees can use the issued ticket attestation as an Ethereum token. If we can maintain and open up the system to other event organisers, it will bring millions of new users into Ethereum. Currently, there are 940 million unique digital ticket holders globally; if we can convert just 5% of them, we can bring in 47 million new users into Ethereum (half of the total Ethereum unique addresses as of this writing). 
...
The email attestation service created by AlphaWallet will allow the attendees to claim their attestations at http://attestation.id with the goal of creating a decentralised ecosystem of attesters for the benefit of enriching smart contract functions and reducing on-chain transactions. These attestations can be reused. 
...
Attestation #1
: Attestation linking ticket with an email address (provided by the ticketing system at the time of purchase).

Attestation #2
: [for crypto users] Attestation linking an Ethereum address with an email address (acquired by the user through a dApp browser).
...
Attestation #2 can be issued by http://attestation.id. The process of acquiring attestation #2 would be a simple guided process for the user. It will be through visiting a website, where the user verifies their email to receive an attestation that will be saved in the dApp browser as a cookie or in the user's wallet (if the wallet can recognise attestations).
...
Furthermore, a ticket ownership proof can be generated from [TokenScript](http://docs.TokenScript.org/) that allows a website to recognise Devcon ticket holders. This is useful for ticket holders to book services or to apply for a discount. We can create a sample website for service providers to copy.

### Process Flow ###
The attached swimlane diagram shows how the different actors and parties involved in the ticketing process would use the solution to prove and verify ticket's authenticity to provide privileged access.
=======
**Process Flow**
...
Any service that is required to verify attendees, like a discount for products, redeemable vouchers, content unlocking, IoT(open door, control screen), collateral for DeFis and etc.
